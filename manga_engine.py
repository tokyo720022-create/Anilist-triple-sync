...
# ==========================================
# ⚙️ 1. SYSTEM CONFIGURATION & SECRETS (MANGA ISOLATED)
# ==========================================
...
# ⚡ Manga-Specific Memory Vaults (Prevents collision with Anime)
DB_SYNC = 'db_manga_sync.json'
DB_MESSAGES = 'db_manga_messages.json'
DB_GHOSTS = 'db_manga_ghosts.json'
DB_VOID = 'db_manga_void.json'
DB_ACHIEVEMENTS = 'db_manga_achievements.json' 
DB_PERFORMANCE_MSG = 'db_manga_performance_msg.json'
DB_THREADS = 'db_manga_threads.json'     
DB_INVENTORY = 'db_manga_inventory.json' 
DB_TIMESTAMP = 'db_manga_timestamp.json' 
...
def generate_cli_board():
    ...
    def format_block(title, data, is_lifetime=False):
        ...
        time_m = data.get('manga_minutes', 0)
...
def refresh_performance_hologram():
    ...
    embed = {
        "title": "⚡ LIVE MANGA PERFORMANCE MONITOR",
        "description": f"```text\n{cli_board}\n```",
        "color": 3447003,
...
def fire_zulip_archive(media_type, title, progress, total_chapters, score):
    if not ZULIP_URL or not ZULIP_EMAIL or not ZULIP_API_KEY: return
    content = f"✅ **Sync Log Executed**\n* **📖 Read Chapter:** {progress} / {total_chapters if total_chapters else '?'}\n* **Current Score:** {score if score else 'Unrated'}"
    payload = {"type": "stream", "to": "Manga-Vault", "topic": title, "content": content}
...
def drop_classified_ui(tier):
    ...
    if tier == 1000: send_discord_alert(WEBHOOK_ACHIEVEMENTS, "💠 1,000 G REACHED (MANGA)", ">> CLASS-A MILESTONE CLEARED <<", 16776960, "https://i.imgur.com/QzXoX1j.gif")
    elif tier == 5000: send_discord_alert(WEBHOOK_ACHIEVEMENTS, "🔥 5,000 G: OVERDRIVE (MANGA)", ">> SYSTEM MAXIMIZED. APEX TIER REACHED <<", 16711680, "https://i.imgur.com/W2dM9Ue.gif")

# ==========================================
# 🔎 6. ANILIST GRAPHQL CORE (MANGA ONLY)
# ==========================================
def fetch_anilist_inventory(username):
    # ⚡ SHIELD: Added type: MANGA to exclusively pull manga data
    query = '''
    query ($userName: String,$page: Int) {
      Page(page: $page, perPage: 50) {
        pageInfo { hasNextPage }
        mediaList(userName: $userName, type: MANGA, sort: [UPDATED_TIME_DESC]) {
          updatedAt
          mediaId progress score status customLists
          media {
            title { romaji english }
            type format chapters duration
            genres tags { name }
    ...
    print(">>> [SYSTEM] INITIATING MANGA DEEP ANALYSIS..." if is_first_run else ">>> [SYSTEM] MANGA DELTA-SYNC ENGAGED...")
        ...
        for item in data.get('mediaList', []):
            try:
                ...
                if active_lists:
                    ...
                    if "isekai" in descriptors: smart_category = "Iseki"
...
if __name__ == '__main__':
    print("=== MAXIMUM OVERDRIVE V2 ENGINE: MANGA CORE SPINNING UP ===")
    ...
    print("=== MAXIMUM OVERDRIVE V2 ENGINE: MANGA CYCLE COMPLETE ===")
