...
2.5. [Historical Backfill Protocol](#25-historical-backfill-protocol)
...
# 2.5 HISTORICAL BACKFILL PROTOCOL

V2 introduces a clean separation between **live delta tracking** and **historical telemetry**.

This distinction is critical when migrating from an older version of Maximum Overdrive.

The normal V2 engine intentionally follows the **Fresh Delta Rule**:
...
Delta = Current - Previous
      │
      ├── 0 → no new event
      └── >0 → record new activity
```

That rule is correct for normal hourly operation, but it creates an important migration
problem when the new `performance/` vault has just been created.

A newly created telemetry vault starts with no historical knowledge.

For example, the engine may already know:

```text
Lifetime Gamerscore = 5028 G
```

while the newly introduced performance history still contains:
...
Those zeros do **not** mean the user has never watched anything.

They mean:

> **The V2 Performance Core has not yet been backfilled with historical data.**

The correct migration solution is a **one-time Historical Backfill Protocol**.

---

## 2.5.1 WHY BACKFILL IS NECESSARY

The live engine should never reinterpret the entire AniList library as new activity every hour.

If it did, a library containing hundreds or thousands of watched episodes could suddenly
produce thousands of duplicate Gamerscore rewards and performance events.

Therefore V2 intentionally separates two modes:

```text
LIVE MODE
Current progress vs previous progress

BACKFILL MODE
Historical AniList inventory → initial performance baseline
```

The backfill is performed once.

After the baseline exists, normal hourly executions return to delta-only processing.

---

## 2.5.2 THE GROUND-ZERO EFFECT

A fresh V2 installation can produce the following apparent mismatch:
...
It can simply mean that the performance vault was created after years of tracking history
already existed elsewhere.

The backfill establishes the missing baseline.

---

## 2.5.3 BACKFILL OBJECTIVE

The one-time migration should populate historical performance values without awarding new
Gamerscore for old activity.
...
NO historical Gamerscore duplication
```

The Gamerscore ledger remains authoritative for score history.

The performance vault becomes authoritative for historical consumption telemetry.

---

## 2.5.4 BACKFILL DATA SOURCES

The preferred historical source is the user's current AniList inventory.

The backfill can use information already returned by the master GraphQL query, including:
...
For anime time estimates, the backfill can use AniList duration metadata when available.

For manga, the configured normalized value remains:
...
## 2.5.5 BACKFILL SAFETY RULE

The historical backfill must be **non-rewarding**.

It should not behave like a normal live delta event.

```text
BACKFILL
   │
   ├──► Performance history   YES
   ├──► Daily historical data YES, when reconstructable
   ├──► Lifetime telemetry    YES
   ├──► README telemetry      YES
   ├──► Hologram telemetry    YES
   ├──► New Gamerscore        NO
   ├──► New completion bonus  NO
   └──► Duplicate alerts      NO
```

This is the most important rule of the migration.

---

## 2.5.6 ONE-TIME BACKFILL WORKFLOW

A safe migration sequence is:

```text
1. Stop or temporarily disable scheduled runs.
2. Back up the current JSON state.
3. Run the backfill script once.
4. Validate generated performance files.
5. Validate lifetime telemetry.
6. Validate README telemetry.
7. Validate Discord Hologram values.
8. Commit the historical baseline.
9. Remove or disable the one-time backfill trigger.
10. Resume normal scheduled execution.
```

The backfill should never remain permanently attached to the hourly scheduler.

---

## 2.5.7 RECOMMENDED BACKFILL FILE

For a clean migration, a dedicated script can be added temporarily:

```text
backfill_performance.py
```

The script should be treated as a migration tool rather than a permanent second engine.

Once migration is complete, it can be retained for audit/reference or removed from the
scheduled workflow.

---

## 2.5.8 BACKFILL PSEUDOCODE

```python
load_anilist_inventory()
...
for media in inventory:
    if media.type == "ANIME":
        historical_anime += media.progress
        historical_minutes += calculate_historical_anime_minutes(media)

    elif media.type == "MANGA":
        historical_manga += media.progress
        historical_minutes += media.progress * 5

write_lifetime_performance(
    historical_anime,
    historical_manga,
    historical_minutes
)
...
update_performance_hologram()
```

The exact implementation must follow the production engine's schema.

---

## 2.5.9 BACKFILL DOES NOT RECONSTRUCT EVERY DATE AUTOMATICALLY

A current AniList progress value tells the engine **how much has been consumed**, but it does
not necessarily tell the engine the exact calendar date of every historical episode or chapter.

Therefore a simple inventory backfill can safely reconstruct:

```text
Lifetime episode count
Lifetime chapter count
Estimated lifetime minutes
```

But it may not be able to reconstruct:

```text
2025-03-14 episodes watched
2025-03-15 chapters read
2025-03-16 minutes consumed
```

unless additional historical event data exists.

This distinction should remain explicit in the documentation.

---

## 2.5.10 HISTORICAL DAILY RECONSTRUCTION

If the project has older daily JSON files, Zulip logs, or another event archive, the backfill
can be extended to reconstruct daily history.
...
Existing event ledger
        ↓
Existing archived logs
        ↓
AniList current inventory
```

The lower the source appears in the list, the less precise the historical dates become.

---

## 2.5.11 LIFETIME PERFORMANCE BASELINE
...
A conceptual baseline can look like:
...
After migration, the values should represent the historical baseline and the completion flag
should indicate that the one-time procedure has finished.
...
## 2.5.12 BACKFILL COMPLETION FLAG

A completion flag prevents accidental repeated migration.
...
A future scheduled run should see the flag and skip the migration automatically.

---

## 2.5.13 BACKFILL IDENTITY

The migration itself can have a version identifier.

Example:

```text
performance_backfill_v2_0
```

This makes future migrations easier to distinguish from normal engine cycles.

---

## 2.5.14 BACKFILL VS LIVE MODE

The two modes must never be confused.

```text
╔══════════════════════╦══════════════════════╗
║ Historical Backfill  ║ Live Engine          ║
╠══════════════════════╬══════════════════════╣
║ One-time             ║ Recurring             ║
║ Non-rewarding        ║ Rewarding             ║
║ Baseline creation    ║ Delta processing      ║
║ Historical metrics   ║ New events            ║
║ Migration task       ║ Normal operations     ║
╚══════════════════════╩══════════════════════╝
```

This separation is essential for Gamerscore integrity.

---

## 2.5.15 AFTER BACKFILL

After the historical baseline exists, normal operation resumes:

```text
Previous = historical baseline
Current  = latest AniList state
Delta    = only new activity
```

At that point, the performance system can begin accumulating new daily metrics without
recounting the entire historical library on every run.

---

## 2.5.16 README EXPECTATION AFTER BACKFILL

The README may then display:

```text
Lifetime G       → existing Gamerscore
Lifetime Eps     → historical baseline
Lifetime Chp     → historical baseline
Lifetime Minutes → historical estimate
Daily Eps        → today's new activity
Daily Chp        → today's new activity
```

This is the intended relationship between the two systems.

---

## 2.5.17 WHY GAMERSCORE WAS ALREADY CORRECT

Gamerscore already had an independent persistent ledger.

The newly created Performance Vault did not.

Therefore it is entirely possible for:

```text
Gamerscore = historical
Performance = ground zero
```

The backfill exists specifically to bridge that gap without modifying the score ledger.

---

## 2.5.18 BACKFILL COMPLETION CHECK

Before resuming the scheduler, verify:

```text
[ ] performance/lifetime.json exists
[ ] historical anime count is populated
[ ] historical manga count is populated
[ ] historical estimated minutes are populated
[ ] backfill_complete is true
[ ] README telemetry reflects the baseline
[ ] Hologram reflects the baseline
[ ] db_achievements.json was not inflated
[ ] no historical duplicate notifications were emitted
```

---

## 2.5.19 MIGRATION PHILOSOPHY

A migration should initialize missing knowledge.

It should not manufacture new activity.

```text
Migration = establish baseline
Live Sync = detect change
```

That distinction is now a permanent part of the V2 architecture.
...
The V2 Performance Core starts from a clean telemetry state by design.

When upgrading an existing Maximum Overdrive installation, run the **Historical Backfill
Protocol once** so the new Performance Vault learns the historical baseline without creating
fake new Gamerscore or duplicate notifications.
...
**Migration first. Live tracking second.**

That is the intended V2 upgrade path.
