...
def generate_cli_board():
    ...
    def format_block(title, data, is_lifetime=False):
        ...
        time_m = data.get('anime_minutes', 0)
...
def refresh_performance_hologram():
    ...
    embed = {
        "title": "⚡ LIVE ANIME PERFORMANCE MONITOR",
        "description": f"```text\n{cli_board}\n```",
        "color": 3447003,
...
def fetch_anilist_inventory(username):
    ...
    print(">>> [SYSTEM] INITIATING ANIME DEEP ANALYSIS..." if is_first_run else ">>> [SYSTEM] ANIME DELTA-SYNC ENGAGED...")
...
def execute_ghost_radar(ghost_db):
    ...
    for title, data in list(ghost_db.items()):
        res = requests.post('https://graphql.anilist.co', json={'query': '''query ($search: String,$type: MediaType) { Media (search: $search, type:$type) { id } }''', 'variables': {"search": title, "type": data.get("type", "ANIME")}}, headers=HEADERS)
        if res and res.status_code == 200 and res.json().get('data', {}).get('Media'):
            ...
            send_discord_alert(WEBHOOK_LOG, "🔴 GHOST REJECTED / NOT FOUND", f"AniList database rejected or could not locate search query: **{title}** ({data.get('type', 'ANIME')})", 16711680)
        time.sleep(1.5) 
...
if __name__ == '__main__':
    print("=== MAXIMUM OVERDRIVE V2 ENGINE: ANIME CORE SPINNING UP ===")
    execute_48hr_purge()
    ...
    print("=== MAXIMUM OVERDRIVE V2 ENGINE: ANIME CYCLE COMPLETE ===")
