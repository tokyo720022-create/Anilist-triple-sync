...
def main() -> None:
    print("=" * 68)
    print("        MAL <- ANILIST MANGA SHORT DELTA SYNC v2")
    print("=" * 68)
    ...
    sync_mode = os.environ.get("MANGA_SYNC_MODE", "delta").strip().lower()
    force_full = sync_mode in {"full", "initial"}
    first_run = not previous_media

    if force_full:
        print("🚀 FULL MANGA SYNC MODE")
        print("   All current AniList manga will be added/updated on MAL.")
    elif first_run:
        print("🚀 INITIAL MANGA SYNC MODE")
        print("   No previous state exists; all current AniList manga will be added/updated on MAL.")
    else:
    ...
    print(f"✅ Manga selected for MAL add/update: {len(changed_entries)}")
    ...
    for entry in changed_entries:
        ...
        if not mal_id:
            skipped_no_mal_id += 1
            print(f"⏭️ {title}: no MAL ID; skipped and left pending.")
            continue
    ...
    for entry in changed_entries:
        ...
        if previous is not None:
            ...
            # New/failed entries are not advanced into the saved state.
    ...
    print("\n" + "=" * 68)
    print("🚀 MAL MANGA FULL SYNC COMPLETE" if (force_full or first_run) else "⚡ MAL MANGA SHORT DELTA SYNC COMPLETE")
    print("=" * 68)
