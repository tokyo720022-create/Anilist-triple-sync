...
def main() -> None:
    ...
    print(
        f"✅ Manga selected for "
        f"MAL add/update: "
        f"{len(changed_entries)}"
    )

    print(
        f"🗑️ Removed from AniList      : "
    ...
    cursor = max(
        (
            item["updated_at"]
            for item in merged_media.values()
        ),
        default=0,
    )
